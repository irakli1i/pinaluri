...
async function add_to_dom_trending() {
    ...
    trending_el.innerHTML = data.slice(11, 15).map(e => {
        return `
        <div class="card" data-id="${e.id}">
        <div class="img">
            <img src="${image_path + e.poster_path}">
...
async function showSlides(n) {
    ...
    let filteredData = data.filter((item) => item.vote_average >= 7)

    console.log(filteredData)


    if (n > filteredData.length - 1) {
        n = filteredData.length - 1;
        slideIndex = filteredData.length - 1;
    }
    if (n < 1) {
        n = 1;
        slideIndex = 1;
    }
    document.getElementById('slideIdex').innerHTML = `${n} / ${filteredData.length - 1}`;
    document.getElementById('slideImage').src = `${image_path + filteredData[n].poster_path}`;
